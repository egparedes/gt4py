...
from gt4py.gtscript import stencil, function
...
backend = "numpy" #sys.argv[1]


class MyDefaultPipeline(DefaultPipeline):
    def run(self, oir: oir.Stencil) -> oir.Stencil:
        ...
        for step in self.steps:
            ...
            stop_time = time.time()
            print(
                f"\t Done in {(stop_time - step_time):.4} secs (total: {(stop_time - start_time):.4})\n"
            )
        print(f"All done in {(stop_time - start_time):.4} secs\n")
        return oir


#pipeline = MyDefaultPipeline(skip=[HorizontalExecutionMerging])
pipeline = MyDefaultPipeline()

stencil = functools.partial(stencil, backend=backend, format_source=False, oir_pipeline=pipeline)


FloatField = Field[IJK, float]
...
@function
def dim(a, b):
    return a - b if a - b > 0.0 else 0.0
...
def qs_table2_fn(i):
    ...
    if i == 1599:
        ...
        table = (0.05 * (TICE - tem0)) * table + (0.05 * (tem0 - 253.16)) * table_vapor_oneline(
            tem0
        )
        # table2(i - 1)
        tem0 = tem_lower(1598)
...
def melt_snow(pt1, cvm, fac_smlt, qs, ql, qr, q_liq, q_sol, lhi, icp2, mc_air, qv, c_vap, qs_mlt):
    ...
    if qs > 1e-7 and dtmp > 0.0:
        snowfac = (dtmp * 0.1) ** 2
        tmp = qs if 1.0 < snowfac else snowfac * qs  # no limiter on melting above 10 deg c
    ...
    if pt1 < t_sub:
        ...
        if qi > 1.0e-8:
            pidep = (
                ...
                * 349138.78
                * expsubl
                / (iqs2 * den * LAT2 / (0.0243 * RVGAS * pt1**2.0) + 4.42478e4)
            )
        else:
            pidep = 0.0
        if dq > 0.0:
...
def compute_pkz_func(delp, delz, pt, cappa):
    return exp(cappa * log(RDG * delp / delz * pt))


@stencil(externals=externals)
def satadjust(
    peln: FloatField,
    qv: FloatField,
    ql: FloatField,
    ...
    with computation(PARALLEL), interval(...):
        ...
        if __INLINED(hydrostatic):
            den = dp / ((peln[0, 0, 1] - peln) * RDGAS * pt)
        else:
            den = -dp / (GRAV * delz)
        ...
        if do_qa and last_step:
            ...
            if tin < T_WFR:
                ...
                # variability
            mindw = min(1.0, abs(hs) / (10.0 * GRAV))

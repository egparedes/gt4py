...
def test_deduce_type():
    assert xtyping.deduce_type(3) == int

    assert xtyping.deduce_type(None) is type(None)  # noqa: E721  # do not compare types
    assert xtyping.deduce_type(type(None)) is type(None)  # noqa: E721  # do not compare types
    assert xtyping.deduce_type(None, none_as_type=False) is None
    assert xtyping.deduce_type(type(None), none_as_type=False) is None

    assert xtyping.deduce_type(Dict[str, int]) == Dict[str, int]

    assert xtyping.deduce_type({1, 2, 3}) == Set[int]
    assert xtyping.deduce_type(frozenset({"1", "2", "3"})) == FrozenSet[str]

    assert xtyping.deduce_type({"a": [0], "b": [1]}) == Dict[str, List[int]]

    assert xtyping.deduce_type(str) == Type[str]

    class A:
        ...

    assert xtyping.deduce_type(A()) == A
    assert xtyping.deduce_type(A) == Type[A]

    def f1():
        ...

    assert xtyping.deduce_type(f1) == Callable[[], Any]

    def f2(a: int, b: float) -> None:
        ...

    assert xtyping.deduce_type(f2) == Callable[[int, float], type(None)]
    ...
    assert xtyping.deduce_type(f4) == Callable[[int, float], type(None)]
    assert (
        xtyping.deduce_type(f4, annotate_callable_kwargs=True)
        == Annotated[
